...
def plot(e,Eb,Eu,Eo,El,Ea,Et,Ep,Etor,Ef,Ev,Ehb,Ec,r=None,show=False):
    plt.figure(figsize=(15,12))    
    plt.subplot(3,3,1)   
    if r is None:
       plt.plot(Eb,alpha=0.8,linestyle='-',color='b',label='Ebond')
    else:
       plt.plot(r,Eb,alpha=0.8,linestyle='-',color='b',label='Ebond')
    ...
    if r is None:
       ...
       plt.plot(r,Eo,alpha=0.8,linestyle='-',color='b',label='Eover')
    plt.legend(loc='best',edgecolor='yellowgreen')
    ...
    if r is None:
       ...
       plt.plot(r,Eu,alpha=0.8,linestyle='-',color='b',label='Eunder')
    plt.legend(loc='best',edgecolor='yellowgreen')
    ...
    if r is None:
       ...
       plt.plot(r,Ea,alpha=0.8,linestyle='-',color='b',label='Eang') 
    plt.legend(loc='best',edgecolor='yellowgreen')
    ...
    if r is None: 
       ...
       plt.plot(r,Ec,alpha=0.8,linestyle='-',color='b',label='Ecoul')
    plt.legend(loc='best',edgecolor='yellowgreen')
    ...
    if r is None:  
       ...
       plt.plot(r,Et,alpha=0.8,linestyle='-',color='b',label='Etor') 
    plt.legend(loc='best',edgecolor='yellowgreen')
    ...
    if r is None: 
       ...
       plt.plot(r,Ehb,alpha=0.8,linestyle='-',color='b',label='Ehb') 
    plt.legend(loc='best',edgecolor='yellowgreen')
    ...
    if r is None:
       ...
       plt.plot(r,Ev,alpha=0.8,linestyle='-',color='b',label='Evdw')
    plt.legend(loc='best',edgecolor='yellowgreen')
    ...
    if r is None: 
       ...
       plt.plot(r,e,alpha=0.8,linestyle='-',color='b',label='Total Energy')
    plt.legend(loc='best',edgecolor='yellowgreen')
...
def deb_energy(images,atomi=0,atomj=1,r_is_x=False,debframe=[],show=False,nn=True,libfile='ffield.json'):
    ir = IRFF_NP(atoms=images[0],
                 libfile=libfile,
                 nn=nn)
    ir.calculate_Delta(images[0])
    ...
    fcsv.close()
    if r_is_x:
       plot(e,Eb,Eu,Eo,El,Ea,Et,Ep,Etor,Ef,Ev,Ehb,Ec,r=R,show=show)
    else:
       plot(e,Eb,Eu,Eo,El,Ea,Et,Ep,Etor,Ef,Ev,Ehb,Ec,r=None,show=show)
    return e


def deb_bo(images,i=0,j=1,figsize=(8,6),print_=False,show=False,more=False,
           fluctuation=False,u=1.5,l=1.35,libfile='ffield.json',
        ...
        if print_:
           if debug:
              ...
              print('{:3d}  r: {:6.4f} bosi: {:6.4f} bopi: {:6.4f} bopp: {:6.4f} '
                    'bo0: {:6.4f} bo1: {:6.4f} e: {:6.6f}'.format(i_,
                r[-1],bopsi[-1],boppi[-1],boppp[-1],bo0[-1],bo1[-1],ir.ebond[i][j]))  
    emin_ = np.min(eb)
...
def deb_f(images,i=0,j=1,figsize=(8,6),print_=False,show=False,more=False,
           fluctuation=False,u=1.5,l=1.35,
           nn=True,debug=False,bo_p=0,x_distance=False):
    r,bopsi,boppi,boppp,bo0,bo1,eb,esi = [],[],[],[],[],[],[],[]
...
def compare_dft_energy(images,show=False,mol=None,nn=True):
    ir = IRFF_NP(atoms=images[0],
                 libfile='ffield.json',
                 mol=mol,nn=nn)
    ir.calculate_Delta(images[0])
    ...
    for i_,atoms in enumerate(images):  
        ...
        e.append(ir.E)
        print(e_dft[-1],e[-1])
        # print('r: {:6.4f} e: {:6.4f}'.format(ir.r[0][1],Ev[-1]))
        
    plt.figure()     
    
    plt.plot(e,alpha=0.8,linewidth=2,linestyle='-.',color='b',label=r'$E_{ReaxFF-MPNN}$')
