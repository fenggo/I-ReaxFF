    ...
    basis = {'C':['C      3',
              ...
              'Al':['Al     3           # Species label, number of l-shells',
               ...
               'Fe':['Fe 5',
                     ...
                     '   8.0 2.2136430',
                     ' n=3 2 2 E 40 -0.9',
                     '   5.85260682156 3.0385538',
                     ' n=4 0 2 E 40 -0.9',
                     '   9.17876877610 7.1301390',
                     ' n=4 1 1 E 40 -0.9 Q 1.3284039 19.9788401',
                     '   9.17876877610']
                  }
    return basis

def siesta_traj(label='siesta',fdf='in.fdf',out='siesta.out',batch=10000000):
    cwd = getcwd()
    d = MDtoData(structure=label,inp=fdf,out=out,dft='siesta',direc=cwd,batch=batch)
    images = d.get_traj()
    d.close()
    return images
...
def write_siesta_in(Atoms,infdf='in.fdf',
    ...
    if 'KgridCutoff' in kwargs: 
       kgrid=kwargs['KgridCutoff']
    else:
       kgrid = min(r)
       if singlek:
          kgrid=0.0
    ...
    if 'MaxSCFIterations' in kwargs:
       print('MaxSCFIterations        {:d} \n'.format(kwargs['MaxSCFIterations']),file=fdf)
    if 'EnergyShift' in kwargs: 
       print('PAO.EnergyShift  {:f} meV'.format(kwargs['EnergyShift']),file=fdf)

    
    if basistype=='split':                            ### setting basis set and size
       ...
       if 'Basis' in kwargs:
          ...
          for spec in basis:
              for l in basis[spec]:
                  print(l,file=fdf)
          print('%endblock PAO.Basis',file=fdf)      ### end setting Basis set and size
    ...
    if opt!='SinglePoint':
       ...
       if 'MaxForceTol' in kwargs:
          print('MD.MaxForceTol        {:f} eV/Ang '.format(kwargs['MaxForceTol']),file=fdf)
       else:
    ...
    if 'MonkhorstPack' in kwargs: # KgridCutoff
       print('%block kgrid.MonkhorstPack',file=fdf)
       print('{:d} 0 0 {:f}'.format(kwargs['MonkhorstPack'][0],kwargs['MonkhorstPack'][3]),file=fdf)
       print('0 {:d} 0 {:f}'.format(kwargs['MonkhorstPack'][1],kwargs['MonkhorstPack'][3]),file=fdf)
       print('0 0 {:d} {:f}'.format(kwargs['MonkhorstPack'][2],kwargs['MonkhorstPack'][3]),file=fdf)
       print('%endblock kgrid.MonkhorstPack',file=fdf)
    else:
...
def parse_out(outfile='siesta.out',spec={}):
    f  = open(outfile,'r') 
    lines= f.readlines()
    f.close()        
    atom_name = []
    ...
    for i,line in enumerate(lines):
        ...
        if len(l)>0:
           ...
           elif line.startswith('siesta: E_KS(eV)'):
              e = float(line.split()[-1])
           elif line.find('Atomic forces (eV/Ang)')>=0:
              ln = lines[i+1]
              force   = []
    ...
    atoms     = Atoms(atom_name,positions,cell=cell,pbc=[True,True,True])
    calc      = SinglePointCalculator(atoms,energy=e,forces=forces[-1],stress=presses[-1])
    atoms.calc = calc
    return atoms 
