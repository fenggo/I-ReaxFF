...
class ReaxFF(object):
  ...
  def evalute_pop(self):
      if isfile('POP.pkl'):
         f = open('POP.pkl', 'rb') 
         self.populations = pickle.load(f)   
         f.close()  
         # for chrom in self.populations:
         #     self.p_ = chrom.Genes

         self.populations.sort(key=lambda x:x.Loss,reverse=False)
         if self.fromPop:
            self.p_ = self.populations[0].Genes
      else:
         self.populations    = []
  ...
  def memory(self):
      ...
      self.BOtij,self.BOtjk,self.BOtkl,self.fijkl,self.so = {},{},{},{},{}
      ...
      for mol in self.mols:
          mol_ = mol.split('-')[0]
          self.dft_energy[mol] = tf.placeholder(tf.float32,shape=[self.batch],
                                                name='DFT_energy_%s' %mol)

      self.rbd,self.rv,self.qij = {},{},{}
      for bd in self.bonds:
          if self.nbd[bd]>0:
             self.rbd[bd] = tf.placeholder(tf.float32,shape=[self.nbd[bd],self.batch],
                                           name='rbd_%s' %bd)
          if self.nvb[bd]>0:
             self.rv[bd]  = tf.placeholder(tf.float32,shape=[self.nvb[bd],self.batch],
                                           name='rvdw_%s' %bd)
             if self.optword.find('nocoul')<0:
                self.qij[bd] = tf.placeholder(tf.float32,shape=[self.nvb[bd],self.batch],
                                              name='qij_%s' %bd)
      self.theta = {}
      for ang in self.angs:
          if self.nang[ang]>0:
             self.theta[ang] = tf.placeholder(tf.float32,shape=[self.nang[ang],self.batch],
                                              name='theta_%s' %ang)

      self.s_ijk,self.s_jkl,self.cos_w,self.cos2w,self.w={},{},{},{},{}
      for tor in self.tors:
          if self.ntor[tor]>0:
             self.s_ijk[tor] = tf.placeholder(tf.float32,shape=[self.ntor[tor],self.batch],
                                              name='sijk_%s' %tor)
             self.s_jkl[tor] = tf.placeholder(tf.float32,shape=[self.ntor[tor],self.batch],
                                              name='sjkl_%s' %tor)
             self.w[tor]     = tf.placeholder(tf.float32,shape=[self.ntor[tor],self.batch],
                                              name='w_%s' %tor)
      ...
      for hb in self.hbs:
          if self.nhb[hb]>0:
             self.rhb[hb]  = tf.placeholder(tf.float32,shape=[self.nhb[hb],self.batch],
                                            name='rhb_%s' %hb)
             self.frhb[hb] = tf.placeholder(tf.float32,shape=[self.nhb[hb],self.batch],
                                            name='frhb_%s' %hb)
             self.hbthe[hb]= tf.placeholder(tf.float32,shape=[self.nhb[hb],self.batch],
                                            name='hbthe_%s' %hb)
  ...
  def get_loss(self):
      ...
      for mol in self.mols:
          if self.losFunc   == 'n2':
             ...
             self.loss[mol] = tf.losses.absolute_difference(self.dft_energy[mol],self.E[mol])
          elif self.losFunc == 'mse':
             self.loss[mol] = tf.losses.mean_squared_error(self.dft_energy[mol],self.E[mol])
          elif self.losFunc == 'huber':
             self.loss[mol] = tf.losses.huber_loss(self.dft_energy[mol],self.E[mol],delta=self.huber_d)

          sum_edft = tf.reduce_sum(tf.abs(self.dft_energy[mol]-self.max_e[mol]))
          self.accur[mol] = 1.0 - tf.reduce_sum(tf.abs(self.E[mol]-self.dft_energy[mol]))/sum_edft
  ...
  def get_bond_energy(self):
      ...
      self.Delta  = tf.reduce_sum(D_,1,name='Delta')  # without valence i.e. - Val 
      self.SO     = tf.reduce_sum(SO_,1,name='sumover')  
      
      i = 0
      for bd in self.bonds: 
          [atomi,atomj] = bd.split('-') 
          if self.nbd[bd]>0:
             with tf.name_scope('Ebond_'+bd):
                  [atomi,atomj] = bd.split('-') 
                  self.get_ebond(bd)
      ...
      for mol in self.mols:
          with tf.name_scope('Ebond_'+mol):
               self.ebda[mol] = tf.gather_nd(EBDA,self.bdlink[mol])  
               self.ebond[mol]= tf.reduce_sum(self.ebda[mol],0,name='bondenergy')
  ...
  def get_atom_energy(self):
      ...
      for sp in self.spec:
          ...
          self.dang[sp]  = self.D[sp] - self.p['valang_'+sp]

          self.get_elone(sp,self.D[sp]) 
          self.ELONE  = self.EL[sp] if i==0 else tf.concat((self.ELONE,self.EL[sp]),0)
          self.Dlp[sp]= self.D[sp] - self.p['val_'+sp] - self.Delta_lp[sp]
      ...
      self.DPIL= tf.reduce_sum(self.BPI*self.DLP,1,name='DPI') # 
      self.DPI = tf.reduce_sum(self.BPI,1,name='DPI') # *self.DLP
      ...
      for sp in self.spec:
          ...
          self.get_eover(sp,self.D[sp],self.Dpi[sp])
          self.EOVER = self.EOV[sp] if i==0 else tf.concat((self.EOVER,self.EOV[sp]),0)

          self.get_eunder(sp,self.D[sp],self.Dpi[sp])
      ...
      for mol in self.mols:
          with tf.name_scope('Elone_'+mol):
               self.Elone[mol] = tf.gather_nd(self.ELONE,self.atomlink[mol])  
               self.elone[mol] = tf.reduce_sum(self.Elone[mol],0,name='lonepairenergy')

          with tf.name_scope('Eover_'+mol):
               self.Eover[mol] = tf.gather_nd(self.EOVER,self.atomlink[mol])  
               self.eover[mol] = tf.reduce_sum(self.Eover[mol],0,name='overenergy')

          with tf.name_scope('Eunder_'+mol):
               self.Eunder[mol] = tf.gather_nd(self.EUNDER,self.atomlink[mol])  
               self.eunder[mol] = tf.reduce_sum(self.Eunder[mol],0,name='underenergy')
          
          if self.atomic:
             with tf.name_scope('zpe_'+mol):
                  zpe_ = tf.gather_nd(self.EATOM,self.atomlink[mol]) 
                  self.zpe[mol] = tf.reduce_sum(zpe_,name='zpe') 
  ...
  def get_bondorder_nn(self,bd,atomi,atomj):
      ...
      Fi   = self.f_nn('f',bd,self.nbd[bd],[Di_,Dj_,Dbi,Dbj],layer=self.bo_layer[1])
      Fj   = self.f_nn('f',bdr,self.nbd[bd],[Dj_,Di_,Dbj,Dbi],layer=self.bo_layer[1])
      self.F[bd] = Fi*Fj
  ...
  def get_bondorder(self,bd,atomi,atomj):
      ...
      self.F_11[bd] = tf.cond(case1,F1,one)
      self.F_12[bd] = tf.cond(tf.greater_equal(self.p['ovcorr_'+bd],0.0001),F1,one)
      self.F_45[bd] = tf.cond(tf.greater_equal(self.p['corr13_'+bd],0.0001),F45,one)
  ...
  def get_angle_energy(self):
      ...
      self.PBO    = tf.reduce_prod(PBO_,1)
  ...
  def get_eangle(self,ang,atomj,D):
      ...
      with tf.name_scope('Theta0_%s' %ang):
           self.get_theta0(ang)
          ...
          for tor in self.tors:
              if self.ntor[tor]>0:
                 if len(self.torlink[mol][tor])>0:
                    ...
                    i+=1
          if mol in self.Etor:
             self.etor[mol] = tf.reduce_sum(self.Etor[mol],0,name='etor_%s' %mol)
             self.efcon[mol]= tf.reduce_sum(self.EFC[mol],0,name='efcon_%s' %mol)
  ...
  def get_etorsion(self,tor):
      ...
      BOpjk  = tf.gather_nd(self.BOPI,self.tjk[tor]) 
      #   different from reaxff manual
      self.expv2[tor] = tf.exp(self.p['tor1_'+tor]*tf.square(2.0-BOpjk-self.f_11[tor])) 

      self.cos3w[tor] = tf.cos(3.0*self.w[tor])
      v1 = 0.5*self.p['V1_'+tor]*(1.0+self.cos_w[tor])   
      v2 = 0.5*self.p['V2_'+tor]*self.expv2[tor]*(1.0-self.cos2w[tor])
      v3 = 0.5*self.p['V3_'+tor]*(1.0+self.cos3w[tor])

      self.ETOR[tor]=self.fijkl[tor]*self.f_10[tor]*self.s_ijk[tor]*self.s_jkl[tor]*(v1+v2+v3)


  def f10(self,tor):
      with tf.name_scope('f10_%s' %tor):
              ...
              for k in range(-1,2):
                  for vb in self.bonds:
                      if self.nvb[vb]>0:
                         ...
                         rv_[vb] = tf.sqrt(tf.reduce_sum(tf.square(self.vr[vb]+box),2))
             ...
             for vb in self.bonds:
                 if self.nvb[vb]>0:
                    if len(self.vlink[mol][vb])>0:
                       ...
                       i+=1
             self.evdw[mol] = tf.reduce_sum(self.Evdw[mol],0,name='evdw_%s' %mol)
             if self.optword.find('nocoul')<0:
                self.ecoul[mol]= tf.reduce_sum(self.Ecou[mol],0,name='ecoul_%s' %mol)


  def get_hb_energy(self):
      for hb in self.hbs:
          if self.nhb[hb]>0:
             with tf.name_scope('ehb_%s' %hb):
                   self.get_ehb(hb)

      for mol in self.mols:
          with tf.name_scope('Ehb_'+mol):
               i = 0
               for hb in self.hbs:
                   if self.nhb[hb]>0:
                      if len(self.hblink[mol][hb])>0:
                         ...
                         i+=1
               if mol in self.Ehb: 
                  self.ehb[mol] = tf.reduce_sum(self.Ehb[mol],0,name='ehb_%s' %mol)
               else: 
  ...
  def clip_prameters(self):
      ...
      for k in self.v:
          ...
          if key == 'n.u.':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.0001,0.02)
          elif key=='cutoff':
             self.p[k] = tf.clip_by_value(self.v[k],0.01,0.2)
          elif key=='ovun5':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],-49.00,49.0)
             ...
             self.p[k] = tf.clip_by_value(self.v[k],-99.0,-0.0001)
          elif key in ['bo2','bo4','bo6']:
             self.p[k] = tf.clip_by_value(self.v[k],2.0,50.0)
          elif key == 'boc1':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.01,6.0)
             ...
             if len(b)==1:
                bd = b[0] +'-' +b[0]
             self.p[k] = tf.clip_by_value(self.v[k],0.95*self.re[bd],1.2*self.re[bd])
          elif key == 'ropi':
             self.p[k] = tf.clip_by_value(self.v[k],0.55*self.p_['rosi_'+vn[1]],0.98*self.p_['rosi_'+vn[1]])
          elif key == 'ropp':
             self.p[k] = tf.clip_by_value(self.v[k],0.55*self.p_['ropi_'+vn[1]],0.98*self.p_['ropi_'+vn[1]])
          elif key == 'rohb':
  ...
  def session(self,learning_rate=3.0-4,method='AdamOptimizer'):
      if self.ncpu is None:
         ...
         self.config = tf.ConfigProto(device_count={'CPU':self.ncpu},
      ...
      if self.interactive:
         ...
         if method=='GradientDescentOptimizer':
            ...
            optimizer = tf.train.MomentumOptimizer(learning_rate,0.9)  #momentum=0.9
  ...
  def reset(self,opt=[],libfile=None):
      ...
      with tf.name_scope('input'):
           self.memory()
      if not self.atomic:
         with tf.name_scope('zero_point_energy'):
              self.set_zpe()
      self.build_graph() 
      self.feed_dict = self.feed_data()   
  ...
  def get_gradient(self,var,gvar):
      gd = tf.gradients(var,gvar)
      if gd[0] is None:
         g = None
      else:
         grad = tf.convert_to_tensor(gd)
         if self.interactive:
  ...
  def get_all_gradient(self):
      tlist = tf.trainable_variables()
      grads = tf.gradients(self.Loss, tlist)
      grad,tn,tl  = [],[],[]
